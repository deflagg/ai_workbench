...
There are a few bugs which will express themselves if the context is pushed too hard, but it's find for normal use and it's a work in progress. The BOS and POS tokens are now missing so there's that. In addition, more optimizations can be made but I'm ok with it for now.  Below are a couple images of the training run's in Weights & Biases.  I manually adjust the hyperparameters to get a good feel for how each affects the graphs and to start to understand what's most important in training runs.
